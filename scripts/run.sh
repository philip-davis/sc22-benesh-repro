...
for iter in `seq 1 ${ntrials}` ; do
    ...
    jsrun -n $nprocs -a 1 -c 1 -r 32 -e prepended -o left-b.${iter}.out.%j -k left-b.err.${iter}.%j singularity exec --bind $MPI_ROOT:$MPI_ROOT,/autofs/nccs-svm1_home1,/autofs/nccs-svm1_home1:/ccs/home,$(pwd):/jobdir --pwd /jobdir ../sc22.sif /benesh/benesh/build/bin/heateq_benesh $xprocdim $yprocdim $xdim $ydim heat1 &
    ...
    jsrun -n $nprocs -a 1 -c 1 -r 32 -e prepended -o right-b.${iter}.out.%j -k right-b.err.${iter}.%j singularity exec --bind $MPI_ROOT:$MPI_ROOT,/autofs/nccs-svm1_home1,/autofs/nccs-svm1_home1:/ccs/home,$(pwd):/jobdir --pwd /jobdir ../sc22.sif /benesh/benesh/build/bin/heateq_benesh $xprocdim $yprocdim $xdim $ydim heat2
...
#for iter in `seq 1 ${ntrials}` ; do
    ...
    export IBRUN_TASKS_PER_NODE=32
    startproc=$(($IBRUN_TASKS_PER_NODE * $nserv))
    jsrun -n $nprocs -a 1 -c 1 -r 32 -e prepended -o left-a.${iter}.out.%j -k left-a.${iter}.%j singularity exec --bind $MPI_ROOT:$MPI_ROOT,/autofs/nccs-svm1_home1,/autofs/nccs-svm1_home1:/ccs/home,$(pwd):/jobdir --pwd /jobdir ../sc22.sif ${adhoc_exe} $xprocdim $yprocdim 0 0 0.625 1.09375 ${xdim} ${ydim} 0.46875 1.09375 &
    echo "started left side"
    leftproc=$!
    ...
    export IBRUN_TASKS_PER_NODE=32
    startproc=$(($nprocs + ($IBRUN_TASKS_PER_NODE * $nserv)))
    jsrun -n $nprocs -a 1 -c 1 -r 32 -e prepended -o right-a.${iter}.out.%j -k right-a.${iter}.%j singularity exec --bind $MPI_ROOT:$MPI_ROOT,/autofs/nccs-svm1_home1,/autofs/nccs-svm1_home1:/ccs/home,$(pwd):/jobdir --pwd /jobdir ../sc22.sif ${adhoc_exe} $xprocdim $yprocdim 0.46875 0 1.09375 1.09375 ${xdim} ${ydim} 0 0.625
    echo "finished right side, waiting on left"
    wait $leftproc
